...
class DarwinLoadControl {
  ...
  DarwinLoadControl({
    ...
    this.shouldWriteFinalOutputToFile = false,
  });
  ...
  DarwinLoadControlMessage _toMessage() => DarwinLoadControlMessage(
        ...
        shouldWriteFinalOutputToFile: shouldWriteFinalOutputToFile,
      );
}
...
abstract class IndexedAudioSource extends AudioSource {
  ...
  IndexedAudioSource({this.tag, this.duration, this.effects});
...
class AudioPipeline {
  ...
  List<AudioEffect> get _audioEffects => <AudioEffect>[
        if (_isAndroid()) ...androidAudioEffects,
        if (_isDarwin()) ...darwinAudioEffects
      ];
...
class DarwinDelay extends AudioEffect with DarwinAudioEffect {
  ...
  DarwinDelay({
    ...
    if (enabled != null) _enabledSubject.add(enabled);

    if (delayTime != null) _secondsDelayTimeSubject.add(delayTime);

    if (feedbackPercent != null) _feedbackPercentSubject.add(feedbackPercent);

    if (lowPassCutoffHz != null) _lowPassCutoffHzSubject.add(lowPassCutoffHz);
...
class DarwinDistortion extends AudioEffect with DarwinAudioEffect {
  ...
  DarwinDistortion({
    bool? enabled,
    DarwinDistortionPreset? preset,
    double? wetDryMix,
    double? preGain,
  }) {
    if (enabled != null) _enabledSubject.add(enabled);

    if (preset != null) _presetSubject.add(preset);

    if (wetDryMix != null) _wetDryMixSubject.add(wetDryMix);

    if (preGain != null) _preGainSubject.add(preGain);
  }
...
/// A Darwin [AudioEffect] that reverb
...
/// An [AudioEffect] for Both Android And IOS that can adjust the gain for different
/// frequency bands of an [AudioPlayer]'s audio signal.
class Equalizer extends AudioEffect with DarwinAudioEffect, AndroidAudioEffect {
  ...
  Equalizer({required DarwinEqualizerParametersMessage darwinMessageParameters})
  ...
  Future<void> _activate() async {
    ...
    if (_isAndroid()) {
      ...
      _parameters = EqualizerParameters._fromAndroidMessage(
          _player!, response.parameters);
    ...
    if (_isDarwin()) {
      ...
      _parameters = EqualizerParameters._fromDarwinMessage(
          _player!, _darwinMessageParameters);
      _parametersCompleter.complete(_parameters);
  ...
  AudioEffectMessage _toMessage() {
    if (_isAndroid()) {
      return AndroidEqualizerMessage(
        enabled: enabled,
    ...
    if (_isDarwin()) {
      return DarwinEqualizerMessage(
        enabled: enabled,
        parameters: _darwinMessageParameters,
      );
    }
...
class EqualizerParameters {
  ...
  static EqualizerParameters _fromAndroidMessage(
      ...
      EqualizerParameters(
        ...
        bands: message.bands
            .map((bandMessage) =>
                EqualizerBand._fromAndroidMessage(player, bandMessage))
            .toList(),
      );
  ...
  static EqualizerParameters _fromDarwinMessage(
      ...
      EqualizerParameters(
        ...
        bands: message.bands
            .map((bandMessage) =>
                EqualizerBand._fromDarwinMessage(player, bandMessage))
            .toList(),
      );
}
...
class EqualizerBand {
  ...
  static EqualizerBand _fromAndroidMessage(
          AudioPlayer player, AndroidEqualizerBandMessage message) =>
      EqualizerBand._(
        player: player,
        index: message.index,
  ...
  static EqualizerBand _fromDarwinMessage(
          AudioPlayer player, DarwinEqualizerBandMessage message) =>
      EqualizerBand._(
        player: player,
        index: message.index,
